...
it("two clients observe identical rolls, receive private chat, and recover a snapshot", async () => {
  ...
  try {
    ...
    await until(() => statesA.length > 0 && statesB.length > 0);
    ...
    await until(() => statesA.length >= 2 && statesB.length >= 2);
    expect(statesA.at(-1)).toEqual(statesB.at(-1));
    ...
    expect(snapshot.pawns).toHaveLength(16);
  } finally {
