...
- **Board rendering is the traditional 15x15 cross shape**, not a simplification. Two earlier attempts at deriving its 52-cell path by hand/manual reasoning both produced non-adjacent cell transitions and were abandoned in favor of a 14x14 square-ring stand-in; the user rejected that stand-in against `designs/apple_inspired_ludo_arena_with_translucent_glass_pieces/screen.png`. It was solved on the third attempt by modeling the board as a graph (15x15 grid, arms = rows/cols 6-8, minus the 4 base quadrants, minus home columns, minus the true center) and computing node degree to verify every track cell has exactly degree 2 before tracing the cycle mechanically, rather than reasoning about adjacency by hand. That surfaced a real structural fact: with 6-wide base quadrants the shared ring has 56 cells, not 52 — resolved by keeping the 4 arm-to-arm corner cells as real, rendered, always-empty cells that are simply never assigned a `pathIndex` (see `components/arena/boardLayout.ts`'s docstring). The resulting 52-cell table was confirmed against the existing game logic before any code was written: entry cells land exactly on `ENTRY_OFFSET`'s {0,13,26,39}, and the derived safe-cell offsets are identical to `SAFE_CELLS` in `lib/board/geometry.ts` — the two independently-described boards agree. `lib/board/geometry.ts` itself (the pgTAP/parity-tested game logic) did not change; this was purely a rendering-layer fix. Home-lane/finished progress is still shown in the status pod, not as on-board cells — that part of the original simplification stands.
- **Design-matching pass (post-M4):** typography (Plus Jakarta Sans via `next/font/google`), the full color/radius/elevation token set, and every component restyled directly from `designs/modern_boardroom/DESIGN.md`'s literal specs — dice as real pips (not a numeral) with a six-roll pulse in the acting player's color, ceramic-disk pawn tokens (perimeter ring + inner motif + contact shadow, offset-stacked when 2+ share a cell), status pods with an SVG countdown ring (player color draining to warning red) plus the numeral PRD 7.2 requires, and buttons matching the spec's height/radius/press-scale, including the roll button shifting to the acting player's own quadrant color. Verified visually via the same headless-browser approach as the M4 playtest (screenshots actually inspected, not just build/lint passing) — this is real, not just plausible-sounding: static analysis had already gone clean before any of it was looked at. **The ceramic-disk pawn styling described here was superseded** by the layout pass below — see that bullet.
- **Arena layout pass (post cross-shape board):** rebuilt `MatchArena` into the 3-column layout from `designs/apple_inspired_ludo_arena_with_translucent_glass_pieces/code.html` — a Match Events sidebar (left), status pods + board (center), and a Control Center (right: dice, Roll Dice, Auto-Roll, a Game Standings list) — collapsing to a single column below `lg`/`xl`. That reference's board itself is just a flat, externally-hosted `<img>` in its markup (not something to extract exact pixels from), but its pawn styling (`.glass-token`/`.glass-{color}`) is real CSS and was ported verbatim into `app/globals.css`, replacing the flat ceramic-disk pawns. The board gained: solid quadrant fills with a low-opacity compass-star emblem, 4 individual nest parking slots per base (each a star badge, matching the reference's corner-parked pawns) instead of one shared nest box, a dense star badge on every home-lane cell, and a colored star badge on the 8 real `SAFE_CELLS` (color derived from which arm the cell sits in — `boardLayout.ts`'s `armColorForCell`, confirmed against the reference: each arm's decorations consistently match its home lane's color). Individual decorative arrow icons from the reference were **not** reproduced (no functional meaning, not worth the one-off placement cost) — **superseded**, they were added in a later pass, see below; board hue values were **not** pixel-sampled from the reference screenshot either, since the image is heavily glow-contaminated by its own pawn shadows — the existing `--quadrant-*` tokens were already a close match on visual inspection and were left as-is. The economy elements in the reference's top bar (a "Coins" pill, "MACOS" window chrome) were deliberately **not** built — PRD non-goals (no pot/rake/payout/virtual currency) already won that conflict earlier in this project. Verified the same way as every UI pass in this section: lint/tsc/build clean, then an actual headless-browser playtest (desktop 3-column, `lg`-collapsed, and 420px mobile widths) with screenshots inspected, not just static analysis.
- **Entry points relocated into each color's own arm (post arm-decoration pass):** per direct instruction, `components/arena/boardLayout.ts`'s `TRACK_POSITIONS` (the rendering-layer 52-cell ring) was rotated by 47 cells so each color's entry sits next to its own base, in its own arm, instead of at the far end of the next color's arm — e.g. red's entry (global cell 0) moved from the top arm (`[5,6]`) to its own left arm (`[6,1]`). `ENTRY_OFFSET` is unchanged (still `0/13/26/39`); only which physical cell each of those numbers points to moved. Since `SAFE_CELLS` is defined as entry+3/entry+8, this necessarily moved the safe-cell *numbers* too — but not their physical positions, which is what was actually asked for: the same 8 pixel-verified physical cells (`designs/board-design.png`) stay safe, just relabeled from `{3,8,16,21,29,34,42,47}` back to `{0,8,13,21,26,34,39,47}` — which happens to be exactly what Section 3 above specified from the start (the entry+3/entry+8 numbers were only ever correct for the *old*, now-superseded ring orientation). Mirrored into a new migration (`20260915012421_relocate_entry_points_own_arm.sql`, overriding `private.ludo_is_safe_cell` — never edit an already-applied migration in place) and both `SAFE_CELLS` test suites. Verified the rotation itself with a throwaway Node script before touching any file (still exactly 52 unique cells, still exactly 4 diagonal arm-to-arm seams now at 4/5, 17/18, 30/31, 43/44 instead of 12/13, 25/26, 38/39, 51/0, and all 4 new entries land adjacent to their own base), then the usual lint/tsc/build + headless-browser playtest (desktop + 420px mobile, screenshots cropped per-arm and compared against `designs/board-design.png`) + full Vitest suite.
- **Arm decoration arrows (post arena layout pass):** added back the curved "entry flow", straight "advance", and diagonal arm-to-arm-handoff arrows the layout pass above had deliberately skipped, matching `designs/board-design.png`'s exact shape (feathered 3-chevron tail + bold triangular head) and exact directions — the curved arrow's head points outward, away from center, matching the reference; the straight and diagonal arrows' directions were already correct. (An intermediate version briefly reversed the curved arrow based on a misreading of "point inward"; reverted after checking the reference image directly.)
