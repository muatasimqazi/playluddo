...
| `request_roll` | `room_id` | Valid only in `awaiting_roll` for the calling seat; generates roll using `pgcrypto`'s `gen_random_bytes`, computes legal moves, advances phase per 6.6 | `NOT_YOUR_TURN`, `INVALID_PHASE` | ✅ M2 |
| `request_move` | `room_id, pawn_id` | Valid only in `awaiting_move`; re-derives legality, applies capture/finish, advances phase or ends the match | `NOT_YOUR_TURN`, `INVALID_PHASE`, `ILLEGAL_MOVE` | ✅ M2 |
...
| `toggle_auto_roll` | `room_id, enabled` | Player opts their own seat into server-chosen actions (5.2) | `SEAT_NOT_CONTROLLED` | ⏳ M3 |
| `reclaim_seat` | `room_id` | Reconnect flow; applied at next phase boundary, not mid-resolution (5.2) | `NOTHING_TO_RECLAIM` | ⏳ M3 |
| `sweep_expired_turns` | *(none, cron-only)* | Finds rooms with `turn_deadline_at < now()`, applies the same auto-roll/auto-move/bot-takeover logic through the shared transition function | — | ⏳ M3 |
| `request_rematch` / `accept_rematch` | `room_id` | Summary-screen flow (5.3); 60s expiry | `ROOM_NOT_IN_SUMMARY` | ⏳ M4 |

Every M2 RPC already re-derives legality/authorization from the database on every call — nothing in it trusts a client-supplied claim — so wiring in the M3/M4 functions above doesn't require revisiting what's built.
...
Every mutating function appends a row to `match_events` in the same transaction, and calls a shared internal `resolve_bot_action(room_id)` helper (the plpgsql port of PRD 6.8's bot-priority rules) for every auto-action path — direct player call and sweep-triggered call must not diverge in behavior, since they call the same function.
...
## 8. Scheduled Sweep Job

- `pg_cron` schedule: `select cron.schedule('sweep-turns', '1 second', $$select sweep_expired_turns()$$);` — every **1 second** (PRD 6.3 — chosen against the "95% within timer + 2s" metric budget). No Edge Function or external scheduler involved; `pg_cron` calls the plpgsql function directly.
- `sweep_expired_turns` must be **idempotent and safe to double-fire** (row lock + deadline re-check handles this, per Section 6).
- Also responsible for: `missedDecisionCount` increment/reset, `inactive` at 2 consecutive misses, `bot` at 3 misses or 45s disconnect, and the 3-minute all-human-absent → `abandoned` cutoff (PRD 5.2).

## 9. Frontend Notes

- Component structure should map directly to the four existing mockups once the Design ↔ PRD punch list (PRD §10.1) is applied — do not build against the current mockups' pot/rake/payout copy, Team Battle/Quick Rush modes, Rankings tab, XP system, or free-text chat; those are flagged for removal.
- No client-side prediction of dice value or move legality (PRD 6.2) — show a pending state on the acting button between request and broadcast confirmation.
- `lib/board`'s legal-move function runs client-side too, but **only** for highlighting — the authoritative check is always the plpgsql function's re-derivation on the server.
...
| M3 | Timers, bots, reconnect | Sweep job, bot-priority function shared across all trigger paths, duplicate-session + reclaim handling | Integration tests for timeout/takeover/reconnect green |
| M4 | Realtime + UI | Broadcast wiring, lobby/arena/summary UI against the *corrected* mockups | Manual playtest: 4-human and 4-bot matches complete end to end |
