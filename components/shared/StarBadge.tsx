...
// Sparkle positions (compass-diagonal, NW/NE/SW/SE) for the 4 small accent
// stars inside CompassEmblem, in the same 0-100 viewBox space.
...
/**
 * The medallion inside each base quadrant in
 * designs/apple_inspired_ludo_arena_with_translucent_glass_pieces — a solid
 * color disc, a white 4-point compass star reaching toward its edge, a
 * small solid-color pentagram at the point where the compass points
 * converge, and 4 tiny white sparkle stars on the diagonals between them.
 */
export function CompassEmblem({ color, className }: { color: PlayerColor; className?: string }) {
  const classes = QUADRANT_CLASSES[color];
  return (
    <svg viewBox="0 0 100 100" className={className} aria-hidden>
      <circle cx="50" cy="50" r="46" className={classes.text} fill="currentColor" />
      <path d="M50 6 L59 41 L94 50 L59 59 L50 94 L41 59 L6 50 L41 41 Z" fill="white" />
      <g transform="translate(41 41) scale(0.75)" className={classes.text} fill="currentColor">
        <path d={STAR_PATH} />
      </g>
      {SPARKLE_POSITIONS.map(([cx, cy], i) => (
        <g key={i} transform={`translate(${cx} ${cy}) scale(0.22)`} fill="white">
          <path d={STAR_PATH} transform="translate(-12 -12)" />
        </g>
      ))}
    </svg>
  );
